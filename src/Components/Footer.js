          ...
          <Button
            ...
            onClick={() => {
              ...
              props.tasks.forEach((task, taskIndex) => {
                for (
                  let agentIndex = 0;
                  agentIndex < props.agents.length;
                  agentIndex++
                ) {
                  dispatch(shuffleThenSortArr(props.agents));
                  if (
                    props.agents[agentIndex].load + task.loadScore <= 15 &&
                    props.agents[agentIndex].available === true &&
                    props.agents[agentIndex].programs.includes(
                      task.associatedProgram
                    )
                  ) {
